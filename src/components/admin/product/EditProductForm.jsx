...
export default function EditProductForm({
    ...
    const [existingImg,setExistingImg]=useState(productData?.imageUrls)
    const [description,setDescription]=useState(productData?.description)
  
    const [selectedTags, setSelectedTags] = useState(productData?.tags.length?productData.tags.map((item)=>({label:item,value:item})):[]);
    ...
    const initialValues = {
      ...
      stockItems:productData?.stockItems|| 0,
      productBoost: productData?.productBoost||"",
      metaPrice:productData?.metaPrice||0,
      status: productData?.status||"active",
      productCategory:productData?.productCategory|| "active",
      category: productData?.category?._id||"",
      location: productData?.location?._id||"",
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
      ...
      if(!files.length&&!existingImg){
        return toast.error("please select atleat one image")
      }
    ...
    const removeImageFromS3=async(img,proId)=>{
      try {
        const response = await adminAxiosInstance.delete("/product/deleteimg/", {
          data: { img: img,proId:proId }, // Pass data in the request body
        });
        // Handle response
        console.log(response.data);
        if(response.data.success){
          setExistingImg(response.data.imgs)
        }
      } catch (error) {
        // Handle error
        console.error(error);
    ...
    >
      {({ setFieldValue }) => (
        <Form>
                      ...
                      {existingImg.map((url, index) => (
                        ...
                        >
                          ...
                          <span
                            ...
                            onClick={() => removeImageFromS3(url,productData._id)}
                          >
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex flex-col mt-3 text-sm">
                    ...
                    styles={customStyles}
                      ...
                      id="productType"
                      name="productType"
                      defaultValue={{label:productData?.productType,value:productData?.productType} }
                  ...
                  <div className="flex flex-col mt-3 text-sm">
                    ...
                    <Field
                      ...
                      className="border mt-1 p-1 focus:ring-gray-500 focus:border-gray-500 border-black rounded-md"
                    >
                      {/* <option value={Formik.values.category}></option> */}
                      {types &&
                        types.map((item) => (
                  ...
                  <div className="flex flex-col my-3 text-sm">
                   ...
                   < CreatableSelect
                    isMulti
                    defaultValue={productData.tags.map((item)=>({label:item,value:item}))}
          ...
          <div className="flex justify-center bg-light-green-700 my-5">
            <button type="submit" className="py-1 px-4 rounded-md text-white">
              Save
            </button>
          </div>
        </Form>
